...
def UC_mean_shift(data, mask, delta=5):
    """
    Global mean shift in matrix form based on the CVPR 2018 paper from UC-Irvine
    ...
    Outputs:
    ========
    data: np.array -- clustering result with consecutive non-negative 
        integers representing distinct instances; same size as the input
    """

    embedding_dim = data.shape[-1]
    iteration = 10

    masked_emb = np.copy(data[mask > 0])
    x = np.copy(masked_emb)
    N = x.shape[0]
    for _ in range(iteration):
        x_row_stack = np.zeros((N, N, embedding_dim))
        x_col_stack = np.zeros((N, N, embedding_dim))
        for i in range(N):
            x_row_stack[i, :, :] = x
            x_col_stack[:, i, :] = x
        S = np.linalg.norm(x_row_stack - x_col_stack, ord=2, axis=2)
        G = np.exp(delta * (-(np.square(S))))
        ones = np.ones(shape=(N, 1))
        d = np.matmul(G, ones)
        q = np.divide(1.0, d)
        q = np.reshape(q, [-1])
        P = np.matmul(G, np.diag(q))
        x = np.matmul(np.transpose(P), x)
    data[mask > 0] = np.copy(x)
    data[mask == 0] = 0
    return data
